  ...
  return (
    <main>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-title-md2 mb-1.5 font-bold text-black dark:text-white">
      ...
      {subsLoading ? (
        <div>Loading...</div>
      ) : subsData && subsData.userSubscriptions.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6 2xl:gap-7.5">
          {subsData.userSubscriptions.map((sub) => (
            <div key={sub.id} className="shadow-default dark:bg-boxdark rounded-sm border border-stroke bg-white p-4 dark:border-strokedark md:p-6 xl:p-7.5">
              <Image
                width={64}
                height={64}
                alt="zohologo"
              ...
              />
              <h4 className="mt-5 mb-2 font-medium">{sub.plan.name}</h4>
